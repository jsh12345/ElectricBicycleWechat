$(function(){
	...
	getCurrentUser();
	var data = "";
	var url = baseurl + "/audit/findUnAuditOrder";
...
function getGetCurrentUserResult(result){
	...
	if(data.name=='' || data.name==null || data.currentAccount==null){
		...
		var accountType = data.type;//账号类型
		 if(accountType == 'everybody' || accountType == 'audit' || accountType == 'checkaudit' || accountType == 'everyaudit'){//经销商
			
		}else{
			alert("您没有审核发货通知单的权限，请登录相应的财务账号");
									...
									"<p class='order-item-fare pull-right'>总数量："+s.substring(0,s.indexOf("."))+"</p>" +			
											"<p class='order-item-prince '>总金额：￥"+data[i].local_totalamt+"</p>" +
															"</div></div></div></a>" +
																	...
																	"<button class='btn btn-sm btn-default' onclick=\"auditOrder('"+data[i].comp_id+"','"+data[i].bill_no+"','"+data[i].bill_date+"')\">确认订单</button></div></div>" 	
...
function auditOrder(comp_id,bill_no,bill_date){
	var data = "comp_id=" + comp_id + "&bill_no=" + bill_no + "&bill_date=" + bill_date;
	
	var url = baseurl + "/audit/secondAuditOrder";
	sendRequest("post" , url , data , auditResult);
}
